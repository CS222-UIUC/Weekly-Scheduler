...
def within_30_minutes(event1: Event, event2: Event):
    between = datetime.combine(today, event1.timeslot.end_time) - datetime.combine(today, event2.timeslot.start_time)
...
def define_constraints(constraint_factory: ConstraintFactory):
    return [
        # Hard constraints
        time_conflict(constraint_factory),
        # Soft constraints
...
def time_conflict(constraint_factory: ConstraintFactory):
    # A room can accommodate at most one lesson at the same time.
    return constraint_factory \
        .for_each(Event) \
        .join(Event,
              # ... in the same timeslot ...
              Joiners.equal(lambda event: event.timeslot),
              # form unique pairs
              Joiners.less_than(lambda event: event.name)
              ) \
